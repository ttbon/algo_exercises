...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    //check if subtree count N is consistent -- EDIT IS HERE!!!!
    private boolean isBinaryTree(Node x){
        if (x == null){
            return false;
        }
        int size_l = this._isBinaryTree(x.left);
        int size_r = this._isBinaryTree(x.right);

        //inconsistency anywhere in subtrees
        if (size_l < 0 || size_r < 0){
            return false;
        }

        //inconsistency at current level
        if (x.size != (size_l + size_r + 1)){
            return false;
        }
    ...
    private int _isBinaryTree(Node x){
        if (x == null){
            return 0;
        }
        int size_l = this._isBinaryTree(x.left);
